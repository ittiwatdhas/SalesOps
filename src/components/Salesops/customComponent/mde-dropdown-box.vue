<template>
  <div class="md-dropdown-box" @mouseleave="mouseLeave">
    <md-input-container id="w-7" md-theme class="dropdown-box" @click.native="getWidth($event)">
        ...
        <li class="md-list-item" v-for="(item,index) in data" :key="'cg'+index">
          <div class="md-list-item-row" @click="choose(item,index)">
            <div style="long-row">
              ...
              <span class="descript">{{item.corp_name}}</span>
            </div>
          </div>
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    getWidth(e) {
      ...
      this.widthTag = document.getElementById('w-7').offsetWidth
    },
    openAutoCom() {
