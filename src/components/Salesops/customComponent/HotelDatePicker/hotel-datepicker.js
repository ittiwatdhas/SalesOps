...
var HotelDatepicker = (function() {
  ...
  var HotelDatepicker = function HotelDatepicker(input, options) {
    ...
    this.minNights = opts.minNights;
    this.maxNights = opts.maxNights;
    this.selectForward = opts.selectForward || false;
    this.disabledDates = opts.disabledDates || [];
    this.enableCheckout = opts.enableCheckout || false;
    this.container = opts.container || "";
    this.animationSpeed = opts.animationSpeed || ".5s";
    this.hoveringTooltip = opts.hoveringTooltip; // Or a function
  ...
  HotelDatepicker.prototype.addListeners = function addListeners() {
    ...
    document.addEventListener("mouseover", function(evt) {
      return this$1.documentHover(evt);
    });

    // Add a mouseout event listener to the document. This will help us to:
    // 1 - Hide the tooltip on the mouseout event on days
    document.addEventListener("mouseout", function(evt) {
      return this$1.documentMouseOut(evt);
    });
  ...
  HotelDatepicker.prototype.createDatepickerDomString = function createDatepickerDomString() {
    ...
    for (var i = 1; i <= 2; i++) {
      html +=
        ...
        i +
        '"><thead><tr class="datepicker__month-caption">';
      if (i == 1) {
        html +=
          '<th><span class="datepicker__month-button datepicker__month-button--prev" month="' +
          i +
          '">&lt;</span></th>';
      } else {
        html +=
          '<th><span style="display:none" class="datepicker__month-button datepicker__month-button--prev" month="' +
          i +
          '">&lt;</span></th>';
      }
      html += '<th colspan="5" class="datepicker__month-name"></th>';
      if (i == 2) {
        html +=
          '<th><span class="datepicker__month-button datepicker__month-button--next" month="' +
          i +
          '">&gt;</span></th>';
      } else {
        html +=
          '<th><span style="display:none" class="datepicker__month-button datepicker__month-button--next" month="' +
          i +
          '">&gt;</span></th>';
      }
      html +=
        '</tr><tr class="datepicker__week-days">' +
  ...
  HotelDatepicker.prototype.createMonthDomString = function createMonthDomString(
    _date
  ) {
    var this$1 = this;
    ...
    for (var week = 0; week < 6; week++) {
      ...
      for (var i$2 = 0; i$2 < 7; i$2++) {
        ...
        if (_day$2.valid && this$1.disabledDates.length > 0) {
          if (
            this$1.disabledDates.indexOf(
              this$1.getDateString(_day$2.time, "YYYY-MM-DD")
            ) > -1
          ) {
            _day$2.valid = false;
            isDisabled = true;
        ...
        var dayAttributes = {
          ...
          class:
            ...
            _day$2.type +
            " datepicker__month-day--" +
            (_day$2.valid ? "valid" : "invalid") +
            " " +
            (isToday ? "datepicker__month-day--today" : "") +
            " " +
            (isDisabled ? "datepicker__month-day--disabled" : "") +
            " " +
            (isDisabled && this$1.enableCheckout && flag === 1
  ...
  HotelDatepicker.prototype.isValidDate = function isValidDate(time) {
    ...
    if (this.start && !this.end) {
      ...
      if (
        ...
        (this.minDays > 0 && this.countDays(time, this.start) < this.minDays)
      ) {
        return false;
      }
  ...
  HotelDatepicker.prototype.goToNextMonth = function goToNextMonth(e) {
    ...
    d.setMonth(d.getMonth() - 0, 0);

    this.showMonth(d, 1);
    this.showMonth(nextMonth, 2);
  ...
  HotelDatepicker.prototype.disableNextPrevButtons = function disableNextPrevButtons() {
    ...
    var nextButton = this.datepicker.getElementsByClassName(
      "datepicker__month-button--next"
    )[0];

    // Second month "previous" month button
    var prevButton = this.datepicker.getElementsByClassName(
      "datepicker__month-button--prev"
    )[1];
