<template>
  <div>
    <md-card
      class="month-body"
      :style="{'width' : show_range ?'560px !important':'350px !important' ,
          ...
          <md-layout md-align="center" md-flex="40">
            <md-button class="pr-20 pl-10" @click="openYearPicker">
              <label class="md-body-2 pl-18">
                <span class="body-primary-text pt-10">{{year == '' ? 'YEAR' : year}}</span>
              </label>
            </md-button>
          </md-layout>
        ...
        <md-layout class="ph-10 pv-10">
          <div v-if="show_mh == true" style="display: contents;">
            <md-layout
              ...
              :key="items.id"
              :class="month == items.id ? 'month-layout current' : 'month-layout'"
            >
              <div
