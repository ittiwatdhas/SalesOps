  ...
  <div id="discontinued">
    ...
    <div class="table-bar">
      <div class="head-table-bar">
        ...
        <div
          class="sorting"
          style="width:7%;justify-content: flex-end"
        ...
        <div class="block center" style="width:6%">Salesman</div>
        <div
          class="sorting"
        ...
        <div class="block center" style="width:5%">WH</div>
        <div class="block center" style="width:7%">Price</div>
        <div
          class="sorting"
        ...
        <div class="block right" style="width:4%">DRPR</div>
        <div class="block right" style="width:4%">DNSS</div>
        <div class="block right" style="width:4%">DNPC</div>
        <div class="block right" style="width:4%">DNMT</div>
      </div>
      <div class="body-table-bar">
        ...
        <div
          ...
          :key="'discontinued-' + index"
          @click="selected(row, 'product')"
        >
          <div class="block" style="width:3%">{{ fmInt(index + 1) }}</div>
          <div class="block left" style="width:17%;display:flex">
            <div>
              {{ row.item_code }}
            </div>
            <div style="padding-left:20px;font-family:kanit">{{ row.item_name }}</div>
          </div>
          <div class="block center kanit" style="width:6%">
            {{ row.unit }}
          </div>
          <div class="block right" style="width:7%;padding-right:45px">
            {{ fmInt(row.qty) }}
          </div>
          ...
          <div class="block center" style="width:11%">
            {{ fmDate(row.date_time) }}
          </div>
          <div class="block center" style="width:5%">
            {{ fm(row.wh) }}
          </div>
          <div class="block right" style="width:7%">
            {{ fm(row.price) }}
          </div>
          <div class="block right" style="width:11%;padding-right:45px">
            {{ fm(row.total_value) }}
          </div>
          <div class="block right" style="width:4%">
            {{ fmInt(row.drpr) }}
          </div>
          <div class="block right" style="width:4%">
            {{ fmInt(row.dnss) }}
      ...
      <div class="footer-table-bar">
        ...
        <div class="block" style="width:17%;color:#2A2A2A">
          {{ fmInt(data.length) }}&nbsp;&nbsp;Items
        </div>
        <div class="block center" style="width:6%;color:#2A2A2A">TOTAL</div>
        <div
          class="block right"
          style="width:7%;color:#2A2A2A;padding-right:45px"
        >
          {{ fmInt(sum('qty')) }}
        </div>
        ...
        <div class="block right" style="width:7%;color:#2A2A2A">
          {{ fm(sum('price')) }}
        </div>
        <div
          class="block right"
          style="width:11%;color:#2A2A2A;padding-right:45px"
        >
          {{ fm(sum('total_value')) }}
        </div>
        <div class="block right" style="width:4%">
