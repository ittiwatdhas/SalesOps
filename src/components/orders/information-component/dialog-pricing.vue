                    ...
                    <div id="divBodyUpdate" class="display">
                        ...
                        <div v-else :class="'row-bg-orange'" v-for="(item, updateIndex) in dataTable" :key="updateIndex">
                            <div style="width: 32%;" class="-text-left nowrap-title -font-400">
                                <span>{{ item.pricetype_code }} {{ (pricetype[item.pricetype_code]) ? " : " + pricetype[item.pricetype_code] : "" }}</span>
                            </div>
                            <div style="width: 18%;" class="-text-right">{{ formatNumber(item.price, 2) }}</div>
                            <div style="width: 18%;" class="-text-right">{{ formatNumber(item.selling_conv_price, 2) }}</div>
                            <div style="width: 20%; padding-left: 25px;" class="-text-center">{{ convertDate(item.effective_date) }}</div>
                            <div style="width: 12%;" class="-text-center">{{ convertDate(item.expiry_date) }}</div>
                        </div>
                    </div>
                </div>
            </div>
