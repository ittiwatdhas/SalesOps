<template>
  <div class="md-search-box" @mouseleave="mouseLeave">
    <md-input-container md-theme class="search-box" @click.native="getWidth($event)">
      <md-button class="md-icon-button" @click="performSearch">
        <md-icon style="padding-top:1px;">search</md-icon>
      </md-button>
      <!-- @focus="openAutoCom" -->
      <!-- @keypress.native="showAutocom = true" -->
      <md-input id="my_back_search" :placeholder="placeholder" v-on:change="filter" @focus.native="$event.target.select()"
        :value="value" :class="tempId" @keypress.native="submit"
        @blur="other"></md-input>
  ...
  methods: {
    ...
    filter(param) {
      if (param.length > 2) {
        ...
        // this.$store.dispatch('summaryorders/searchSummaryCust', param)
