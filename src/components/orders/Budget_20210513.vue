<template>
  <div id="pageBudget">
              ...
              <md-card class="tap-table">
                <md-layout md-flex="100" style="flex:unset">
                  ...
                  <div>
                      ...
                      <md-card class="sales-period-view-box" @click.native="openMonth">
                        <md-card-content>
                          <div class="card-title">
                            <span>{{disSelectedDate.month}} {{disSelectedDate.year}}</span>
                          </div>
                    ...
                    <div style="padding-left: 10px;font-family:roboto;font-size:12px;text-align: left;padding-top: 5px;color: #A8A8A8;">
                ...
                <div
                  style="width: 100%; display: flex; font-size: 16px; font-family:roboto;font-weight: bold; margin-top: 15px;padding-top: 10px;padding-bottom: 10px;"
                >
                  ...
                  >{{formatNumber(totalDis_limit)}}</div>
                  <div
                    class="number pt-2 text-right"
                    style="width: 11%; padding-right:10px;"
                  >{{formatNumber(totalDis_used)}}</div>
                  <div
                    class="number pt-2 text-right"
                    style="width: 11%; padding-right:10px;"
                  >{{formatNumber(totalDis_balance)}}</div>
                  <div class="text-left" style=" width: 5%;"></div>
                </div>

                <div :class="'head-table'" id="moreClick">
                  <div class="text-left" style="width: 25%;">Name</div>
                  <div class="text-left" style="width: 10%">Code</div>
                  <div class="text-left" style="width: 17%">Department</div>
                  <div class="text-left" style="width: 10%">type</div>
                ...
                >No Discount Budget</div>
                <div
                  class="detail-blog"
                  style="height: calc(100vh - 330px); padding-left: 0px; margin-right: 0px;"
                >
                  <div
                    v-for="(items) in dataDiscount"
                    :key="items.code"
                    class="content-table"
                  ...
                  >
                    <div
                      class="number pt-2"
                      style="width: 25%; font-family:kanit;font-size:16px;font-weight: normal;"
                    >{{items.name}}</div>
                    <div class="number pt-2" style="width: 10%">{{items.role_code}}</div>
                    <div class="number pt-2" style="width: 17%">{{items.department}}</div>
                    <div class="number pt-2" style="width: 10%">{{items.type}}</div>
                    <div
                      class="number pt-2 text-right"
                      style="width: 11%; padding-right:10px; font-size:13px;font-family:roboto"
                    >
            ...
            <md-layout md-flex="100" md-align="center">
              ...
              <md-card class="tap-table">
                  ...
                  >
                    <div style="width:40%">
                      <div style="display: -webkit-box;width:94%">
                        <div class="sales-period-box" @mouseleave="purMouseLeave">
                          <md-card
                          ...
                          >
                            <md-card-content>
                              <div class="card-title">
                                <div
                                  :style="{'font-size' : purSelectedDate.year == ''?'12px':'18px',
                                'font-weight' : purSelectedDate.year == ''?'600':'' ,
                                'font-family' :'Roboto',
                                'width' :'100%',
                                }"
                                >
                                  <span>{{purSelectedDate.month_name}} {{purSelectedDate.year}}</span>
                                </div>
                              </div>
                              <md-icon>expand_more</md-icon>
                            </md-card-content>
                    ...
                    <div style="padding-left:15px;width:45%" class="pt-5">
                      <div class="md-dropdown-box" @mouseleave="mouseLeaveDropdow">
                        <md-input-container
                          md-theme
                          class="dropdown-box"
                        ...
                        >
                          <div class="no-data-promo" v-if="dropdown.length == 0 ">
                            <span>No data....</span>
                          </div>
                          <ul class="md-list" v-else>
                            <li
                              ...
                              :key="'cg'+index"
                            >
                              <div class="md-list-item-row" @click="selectLevelSales(item)">
                                <div style="long-row">
                ...
                >
                  ...
                  >{{formatNumber(totalPur_amount)}}</div>
                  <div
                    class="number pt-2 text-right"
                    style="width: 10%; padding-right:10px;"
                  >{{formatNumber(totalPur_netamount)}}</div>
                  <div
                    class="number pt-2 text-right"
                    style="width: 10%; padding-right:10px;"
                  >{{formatNumber(totalPur_budget)}}</div>
                ...
                >
                  <div
                    ...
                    style="font-family:roboto;padding-top:7px"
                  >
                    <div class="number pt-2" style="width: 9%">{{items.po_no}}</div>
                    <div class="number pt-2" style="width: 8%">{{showTime(items.po_date)}}</div>
                    <div class="number pt-2" style="width: 8%">{{showTime(items.apv_apvdate)}}</div>
                    <div class="number pt-2" style="width: 8%">{{showTime(items.delivery_date)}}</div>
                    <div
                      class="descript pt-2"
                      style="width: 15%;font-family:kanit;white-space: nowrap;text-overflow: ellipsis;padding-right:8px;overflow: hidden;"
                    >
                      {{items.cust_code}}
                      ...
                      <md-tooltip md-direction="top">
                        ...
                        >{{items.cust_code}}&nbsp;{{items.customer_name_th}}</span>
                      </md-tooltip>
                    </div>
                    <div
                      class="descript pt-2"
                      style="width: 14%;font-family:kanit;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;padding-right:8px;"
                    >
                      {{items.sm_code}}
                      <span
                        style="font-family:kanit"
                      ...
                      <md-tooltip md-direction="top">
                        <span
                          style="font-family:kanit"
                        >{{items.sm_code}}&nbsp;{{items.first_name}}&nbsp;{{items.last_name}}</span>
                      </md-tooltip>
                    </div>
                    <div
                      class="number pt-2 text-right"
                    ...
                    >{{formatNumber(items.sum_budget)}}</div>
    ...
    <md-dialog ref="pageBudgetDG" id="pageBudgetDG" class="md-orders-view">
      ...
      <md-orders-view @close="closeDG" :data="detail" @read="readComment" @reply="replyCommentDG"></md-orders-view>
    </md-dialog>
  </div>
</template>
<script>
