    ...
    <div class="body-cons">
      <div class="box-1">
        <div style="display:flex;width:100%;background: #efefef">
          <div style="width:25%;" class="box-1-head">INVOICE TRANSACTIONS LIST</div>
          <div
          v-if="currentStep ==3"
            style="width: 75%;padding-top:5px;padding-right:15px"
            v-click-outside="closeDivSearch"
          >
            <div class="salesops-input" style="display:flex;width:100%;">
              <md-icon
                style="cursor: pointer; color: #A8A8A8;margin-right:10px"
                @click.native="searchProductItem(searchTxtProduct , 'search')"
              >playlist_add</md-icon>
              <input
                type="text"
                v-model="searchTxtProduct"
                placeholder="Searching for invoice, purchase orders, customer …"
                @keyup.enter="searchProductItem(searchTxtProduct , 'search')"
              />
            </div>
            ...
            <div v-show="showDisplaySearch">
              <md-card
                ...
                style="position: absolute; top: 140px; margin-left: 2px;width: 750px;"
              >
                <div
                  style="width:100%; display:flex; text-align:end; border-bottom: 1px solid rgba(201, 201, 201, 0.57);"
                >
                  <div style="width:60%; padding-top:20px; padding-right:30px; ">SALES CYCLE</div>
                  <div style="width:40%; padding:5px 10px 5px 0px; ">
                    <div
                      style="border: 1px solid rgba(201, 201, 201, 0.57); padding:0px 20px 0px 30px; border-radius: 5px;"
                    >
                      <md-input-container
                        style="margin:0px; padding:5px 0px 5px 0px; min-height:30px"
                      >
                        <md-select v-model="datas.head.cycle_id">
                          <md-option
                            v-for="(row,index) in datas.head.cycle"
                            :key="'cycle-'+row.cycle"
                            :value="row.cycle"
                            @click.native="chooseCycle(row.cycle)"
                          >{{formatterDate(row.start_date)}} - {{formatterDate(row.end_date)}}</md-option>
                        </md-select>
                      </md-input-container>
                    </div>
                  </div>
                </div>
                <div  style="display: flex; width: 100%;padding-top: 5px;padding-bottom: 10px;">
                    <div style="width: 30%; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-family:kanit;padding-left: 8%;">Customers</div>
                    <div style="text-align: center; font-size: 12px; width: 15%;">Invoices</div>
                    <div style="text-align: center; font-size: 12px; width: 12%;">Date</div>
                    <div style="text-align: center; font-size: 12px; width: 15%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">PO NO.</div>
                    <div style="text-align: center; font-size: 12px; width: 12%;">Date</div>
                    <div style="text-align: center; font-size: 12px; width: 14%;">Amount</div>
                ...
                <md-card-content style="padding-top:0px">
                  ...
                  <div
                    v-if="dataSearchProduct.length == 0"
                    style="color:#41414180;text-align:center;width:100%;padding:10px 0px;font-size:15px"
                  >No data...</div>
                  
                  <template v-for="(item, rIndex) in dataSearchProduct">
                    
                    <div
                      :key="`${rIndex}-${item.inv_no}`"
                      style="display: flex; width: 100%; padding: 6px 20px 7px 20px;"
                      :class="'row-bg-gray'"
                    >
                      <label class="-salesops-checkbox-container" style="width: 5%;">
                        <input
                          ...
                          :value="item.inv_no"
                          @click="
                                add(item.cust_code+'|'+item.inv_no , item)
                              "
                        />
                      ...
                      <div
                        class="font-kanit"
                        style="width: 25%; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-family:kanit"
                      >{{ item.cust_code }}&nbsp;:&nbsp;{{item.customer_name_th}}</div>
                      <div
                        class="font-kanit"
                        style="text-align: center; font-size: 12px; width: 15%;"
                      >{{ item.inv_no }}</div>
                      <div
                        class="font-kanit"
                        style="text-align: center; font-size: 12px; width: 12%;"
                      >{{ formatterDate(item.inv_date) }}</div>
                      <div
                        class="font-kanit"
                        style="text-align: center; font-size: 12px; width: 17%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                      >{{ item.po_no }}</div>
                      <div
                        class="font-kanit"
                        style="text-align: center; font-size: 12px; width: 12%;"
                      >{{ formatterDate(item.po_date) }}</div>
                      <div
                        class="font-kanit"
                        style="text-align: right; font-size: 12px; color: #41414180; width: 14%;"
                      >{{ fmFull(item.total_netamount, 0) }}</div>
                    </div>
                  </template>
                </md-card-content>
              </md-card>
...
export default {
  ...
  methods: {
    ...
    add(itemCode, record) {
      let rsMaster = this.datas.head;
      var rs2 = _.find(rsMaster.cycle, function(o) {
        return o.cycle == rsMaster.cycle_id;
      });
      if (!_.isUndefined(rs2)) {
        record.year = rs2.year;
      }
      record.cycle = this.datas.head.cycle_id;
      record.net_value = record.total_netamount;
      this.$emit("add", itemCode, record);
    },
    del(itemCode, record, row) {
      record.cust_code = row.cust_code;
      record.customer_name_th = row.customer_name_th;
      this.$emit("del", itemCode, record);
    },
    chooseCycle(id) {
      this.searchProductItem(this.searchTxtProduct, "cycle");
      // this.$emit("cycle", id);
    },
    ...
    searchProductItem(txtSearch, mode) {
      this.dataSearchProduct = [];
      this.showDisplaySearch = true;
      this.loadingSearch = true;
      let master = _.cloneDeep(this.datas.head);
      let obj = {};
      let key = master.cycle.findIndex(o => {
        return master.cycle_id === o.cycle;
      });
      if (mode === "search") {
        obj = {
          keyword: _.trim(txtSearch),
          sm_code: master.sm_code,
          year: key > -1 ? master.cycle[key].year : master.cycle[0].year,
          cycle: key > -1 ? master.cycle[key].cycle : master.cycle[0].cycle,
          now_year: this.defaultStep.year,
          now_cycle: this.defaultStep.cycle,
      ...
      service.searchOrderStep3(obj).then(response => {
        let master = this.datas.head.docNo;
        this.dataSearchProduct = []
        for (let i = 0; i < response.length; i++) {
          let key = master.findIndex(o => {
            return response[i].cust_code + "|" + response[i].inv_no === o;
          });
          if (key > -1) {
            response[i].checked = true;
            response[i].checked_origin = true;
            this.dataSearchProduct.push(response[i]);
          } else {
            response[i].checked = false;
            response[i].checked_origin = false;
            this.dataSearchProduct.push(response[i]);
          }
        }
